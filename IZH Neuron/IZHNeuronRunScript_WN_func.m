function Output = IZHNeuronRunScript_WN_func(StimI, PulseAmplitude, FixedParams, SpikeParams, NullClineShapeParams,R_m)
% IZH Neuron Run Script White Noise Stimulus
...
% 
    %StimIVectorName = input('Enter name of Stimulus Vector (in single quotes) > ');
    %StringExp = ['StimI = ', StimIVectorName, ';'];
    %eval(StringExp);
    [junk, iStop] = size(StimI);
    
% % compute the total number of iterations this routine will perform
%iStop = ceil(tSpan*SamplingRate);
tSpan = iStop/SamplingRate;
