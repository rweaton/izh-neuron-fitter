function xout = rk4Theta(x, t, tau, derivsRK, param, FnHandleList)
...
%   FnHandleList = list of required function handles to be passed to derivsRK   
%
%	Output Arguments:
%	xout = new value of x after a step of size tau.  Either scalar or vector.

half_tau = 0.5*tau;
F1 = feval(derivsRK, x, t, tau, param, FnHandleList);
t_half = t + half_tau;
xtemp = x + half_tau*F1;
F2 = feval(derivsRK, xtemp, t_half, tau, param, FnHandleList);
xtemp = x + half_tau*F2;
F3 = feval(derivsRK, xtemp, t_half, tau, param, FnHandleList);
t_full = t + tau;
xtemp = x + tau*F3;
F4 = feval(derivsRK, xtemp, t_full, tau, param, FnHandleList);
